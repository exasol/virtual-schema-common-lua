...
describe("validator", function()
    ...
    describe("raises an error if a value is not a valid Exasol user (in place of all database object IDs): ", function()
        for _, test in ipairs({
            {"foo\"bar", 4}, {"Do_not_allow_'single_quotes'", 14}, {"No spaces", 3}, {"1_starts_with_a_number", 1},
            {"_starts_with_an_underscore", 1}, {"·_starts_with_a_middle_dot_utf8_character", 1}
        }) do
            local id = test[1]
            ...
            it(id, function()
                assert.error_matches(function()
                    validator.validate_user(id)
                end, ".*Invalid character in user name at position " .. first_illegal_character .. ": '" .. id .. "'.*")
            end)
        end
    end)
    ...
    it("raises an error if a user (ID) is longer than 128 unicode characters", function()
        ...
        assert.error_matches(function()
            validator.validate_user(id)
        end, "Identifier too long: user name with 129 characters")
    end)

    it("raises an error if a user (ID) is nil", function()
        assert.error_matches(function()
            validator.validate_user(nil)
        end, ".*Identifier cannot be null %(or Lua nil%): user name.*")
    end)
end)
